...
func DecodeJSONBody(w http.ResponseWriter, r *http.Request, dst interface{}) error {
	...
	if r.Header.Get("Content-Type") != "" { // Only allow requests with application/json as header
		...
		if value != "application/json" {
			...
			return middleware.NewCustomError(http.StatusNotAcceptable, msg)
		}
	}
	...
	if err == nil {
		...
		if err != io.EOF { // Don't allow several JSON objects
			...
			return middleware.NewCustomError(http.StatusBadRequest, msg)
		}
	...
	case err.Error() == "http: request body too large":
		...
		return middleware.NewCustomError(http.StatusRequestEntityTooLarge, msg)
...
// Method that uses go-validator package to generically validate parameters of a struct
